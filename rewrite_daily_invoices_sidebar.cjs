    ...
    <div className={\`flex flex-col h-full bg-[#f8fafc] text-slate-800 \${className}\`} dir="rtl">
      ...
      <div className="bg-white border-b border-slate-200 p-2 shrink-0">
        <div className="flex flex-col gap-2">
          ...
          {/* Quick Tabs - All statuses available for filtering */}
          <div className="flex flex-wrap items-center gap-1.5 pb-1">
            <button
              onClick={() => setActiveFilterTab('all')}
              className={\`px-2.5 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'all' ? 'bg-blue-600 text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}\`}
            >
              الكل ({counts.all})
            </button>
            ...
            <button
              ...
              className={\`px-2 py-1 rounded text-[11px] font-bold flex items-center gap-1 transition-colors \${activeFilterTab === 'unpaid_or_credit' ? 'bg-rose-600 text-white shadow-xs' : 'bg-rose-50 text-rose-700 hover:bg-rose-100'}\`}
            >
              <AlertCircle className="w-3.5 h-3.5" /> ذمم وآجل ({counts.unpaid_or_credit})
            </button>
            <button
              onClick={() => setActiveFilterTab('paid')}
              className={\`px-2 py-1 rounded text-[11px] font-bold flex items-center gap-1 transition-colors \${activeFilterTab === 'paid' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}\`}
            >
            ...
            <button
              ...
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'new' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'}\`}
            >
              جديدة ({counts.new})
            </button>
            <button
              onClick={() => setActiveFilterTab('quotation')}
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'quotation' ? 'bg-slate-700 text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}\`}
            >
              عروض ({counts.quotation})
            </button>
            <button
              onClick={() => setActiveFilterTab('design')}
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'design' ? 'bg-fuchsia-600 text-white shadow-xs' : 'bg-fuchsia-50 text-fuchsia-700 hover:bg-fuchsia-100'}\`}
            >
              تصميم ({counts.design})
            </button>
            <button
              onClick={() => setActiveFilterTab('print_internal')}
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'print_internal' ? 'bg-amber-600 text-white shadow-xs' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'}\`}
            >
              طباعة داخلي ({counts.print_internal})
            </button>
            <button
              onClick={() => setActiveFilterTab('print_external')}
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'print_external' ? 'bg-orange-600 text-white shadow-xs' : 'bg-orange-50 text-orange-700 hover:bg-orange-100'}\`}
            >
              طباعة خارجي ({counts.print_external})
            </button>
            <button
              onClick={() => setActiveFilterTab('ready')}
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'ready' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}\`}
            >
              جاهزة ({counts.ready})
            </button>
            <button
              onClick={() => setActiveFilterTab('delivered')}
              className={\`px-2 py-1 rounded text-[11px] font-bold transition-colors \${activeFilterTab === 'delivered' ? 'bg-blue-600 text-white shadow-xs' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'}\`}
            >
              مسلمة ({counts.delivered})
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden">
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-100 border-b border-slate-200 text-slate-600 font-bold">
                <tr>
                  {/* Removed Invoice Number and Workflow Status per request */}
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="p-2 align-top">
                        <div className="font-bold text-slate-800 whitespace-nowrap">{inv.customerName || 'عميل نقدي'}</div>
                        {inv.subCustomerName && (
                          <div className="text-[10px] text-slate-500 font-semibold flex items-center gap-1 mt-0.5 whitespace-nowrap">
                            <User className="w-3 h-3" /> {inv.subCustomerName}
                          </div>
                        )}
                        {inv.notes && (
                           <div className="mt-1.5 text-[10px] text-slate-600 bg-amber-50 border border-amber-100 p-1.5 rounded-md flex items-start gap-1.5 w-full">
                             <AlignRight className="w-3 h-3 text-amber-500 shrink-0 mt-0.5" />
                             <span className="leading-relaxed">{inv.notes}</span>
                           </div>
                        )}
                      </td>
                      <td className="p-2 text-center align-top font-mono font-bold text-slate-800">
                        {inv.totalAmount.toFixed(2)}
                      </td>
                      <td className="p-2 text-center align-top font-mono font-bold text-emerald-700">
                        {inv.paidAmount.toFixed(2)}
                      </td>
                      <td className="p-2 text-center align-top font-mono font-black text-rose-700">
